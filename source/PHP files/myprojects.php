...
echo"<div ><center>";
		...
		$query="SELECT `project`, `target` FROM images WHERE `observerid`=$userid";
		$result = mysqli_query($link, $query) or die(mysqli_error($link)); 
		$rows=mysqli_num_rows($result);
		if($rows==0){
			echo"You don't seem to have any images yet :(";
		}else{
			...
			while($row = mysqli_fetch_array($result)){ 
				...
				*/
				...
				if($unip){
					$ftpdir="<a href=\"ftp://observatory-server.herts.ac.uk/images/".$userid."\">FTP folder link to your images</a><br>";
				}else{
					$ftpdir="";
				}
